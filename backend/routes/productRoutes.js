...
productRouter.post(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    const product = await newProduct.save(); // save newProduct to the database
...
productRouter.get(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    const priceFilter =
      price && price !== 'all'
        ? {
            // 1-100
            price: {
              $gte: Number(price.split('-')[0]), // 1
              $lte: Number(price.split('-')[1]), // 100
            },
          }
        : {};
    ...
    const products = await Product.find({
      ...
      .sort(sortOrder)
      .skip(pageSize * (page - 1)) // Helps to implement pagination
      .limit(pageSize);
